...
Schema duoc bootstrap trong `DatabaseInitializer` bang cac lenh `CREATE TABLE IF NOT EXISTS` va migration nho. `DBConnection` dung HikariCP; local dung SQLite `database/auction-system.db`, cloud dung PostgreSQL neu co `PGHOST`.
...
Table auto_bid_configs {
  ...
  created_at timestamp [not null]
  updated_at timestamp [not null]
  indexes { (auction_id, bidder_id) [pk] }
}
...
Table wallet_transactions {
  ...
  amount decimal(19,2) [not null]
  transaction_type varchar(50) [not null, note: "DEPOSIT | WITHDRAW | PAYMENT | RECEIPT"]
...
- `idx_auto_bid_configs_auction_id`, `idx_auto_bid_configs_bidder_id`
...
- SQLite khong ho tro mot so cau lenh PostgreSQL (`ALTER COLUMN`, `ADD COLUMN IF NOT EXISTS`), nen `DatabaseInitializer` co nhánh bo qua/chuyen doi khi product name la SQLite.
