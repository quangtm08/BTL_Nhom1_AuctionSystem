...
public class ItemImageRepositoryTest {
    ...
    public void testSaveImageUrls_Success() throws SQLException {
        ...
        repo.saveImageUrls(itemId, java.util.Arrays.asList("http://test.com/img1.png", "  ", null, "http://test.com/img2.png"));
    ...
    public void testFindImageUrlsByItemId_MissingTableException() throws SQLException {
        UUID itemId = UUID.randomUUID();
        when(mockConnection.prepareStatement(anyString())).thenThrow(new SQLException("no such table: item_images", "42P01"));
    ...
    public void testFindImageUrlsByItemId_OtherException() throws SQLException {
        UUID itemId = UUID.randomUUID();
        when(mockConnection.prepareStatement(anyString())).thenThrow(new SQLException("Other DB error", "50000"));

        RuntimeException thrown = assertThrows(RuntimeException.class, () -> repo.findImageUrlsByItemId(itemId));
    ...
    public void testFindImageUrlsByItemIdWithConnection_MissingTableException() throws SQLException {
        UUID itemId = UUID.randomUUID();
        when(mockDataSource.getConnection()).thenThrow(new SQLException("relation item_images does not exist", "42P01"));
