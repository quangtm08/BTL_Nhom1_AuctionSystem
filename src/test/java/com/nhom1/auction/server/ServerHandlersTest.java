...
public class ServerHandlersTest {
    ...
    public void testAuthHandler() throws Exception {
        ...
        // Test Register
        when(
            mockAuthService.register("bob", "bob@mail.com", "pass")
        ).thenReturn(mockUser);
    ...
    public void testAdminHandler() throws Exception {
        ...
        // Delete User
        when(mockAdminService.deleteUser("target-1", "caller-1")).thenReturn(
            "Deleted"
        );
        String delUserReq =
        ...
        // Cancel Auction
        when(mockAdminService.cancelAuction("auc-1", "caller-1")).thenReturn(
            "Canceled"
        );
        String cancelAucReq =
    ...
    public void testAuctionHandler() throws Exception {
        ...
        when(mockAuctionService.createAuction(anyString(), any())).thenReturn(
            mockAuction
        );
        ...
        // List My Listings
        when(
            mockAuctionService.getMyListings(
        ...
        verify(mockAuctionService).deleteAuction(
            "00000000-0000-0000-0000-000000000001",
    ...
    public void testAutoBidHandler() throws Exception {
        ...
        AutoBidConfigResponse configResp = new AutoBidConfigResponse(
            "CONFIG_SAVED"
        );
        ...
        when(mockAutoBidService.getConfig("auc-1", "user-1")).thenReturn(
            detailResp
        );
        String getReq =
        ...
        AutoBidConfigResponse delResp = new AutoBidConfigResponse(
            "CONFIG_DELETED"
        );
        when(mockAutoBidService.deleteConfig("auc-1", "user-1")).thenReturn(
            delResp
        );
        String delReq =
        ...
        // Error paths
        when(mockAutoBidService.saveConfig(any())).thenThrow(
            new IllegalArgumentException("Invalid config")
        );
        String configReqErr =
            "{\"type\":\"AUTO_BID_CONFIG\",\"requestId\":\"r-4\",\"payload\":{\"auctionId\":\"auc-1\",\"bidderId\":\"user-1\",\"maxAmount\":100.0,\"increment\":5.0}}";
        assertTrue(
            router.handleRequest(configReqErr).contains("Invalid config")
        );

        when(mockAutoBidService.getConfig(anyString(), anyString())).thenThrow(
            new IllegalArgumentException("Get config error")
        );
        String getReqErr =
            "{\"type\":\"GET_AUTO_BID_CONFIG\",\"requestId\":\"r-5\",\"payload\":{\"auctionId\":\"auc-1\",\"bidderId\":\"user-1\"}}";
        assertTrue(
            router.handleRequest(getReqErr).contains("Get config error")
        );

        when(
            mockAutoBidService.deleteConfig(anyString(), anyString())
        ...
        assertTrue(
            router.handleRequest(delReqErr).contains("Delete config error")
        );
    }
    ...
    public void testBidHandler() throws Exception {
        ...
        String placeBidReq = String.format(
        ...
        String listReq =
            "{\"type\":\"LIST_AUCTIONS\",\"requestId\":\"r-2\",\"payload\":{}}";
        String resp2 = router.handleRequest(listReq);
        assertTrue(resp2.contains("success"));
        ...
        String getDetailReq = String.format(
        ...
        String listMyBidsReq = String.format(
    ...
    public void testPaymentHandler() throws Exception {
        ...
        when(mockPaymentService.processPayment("auc-1", "user-1")).thenReturn(
            ppr
        );
        ...
        PendingPaymentsResponse pprs = new PendingPaymentsResponse(
            Collections.emptyList()
        );
    ...
    public void testClientHandlerAndRegistry() throws Exception {
        ...
        when(mockRouter.handleRequest(anyString())).thenReturn(
            "{\"success\":true}"
        );
    ...
    public void testHandlersErrorPaths() throws Exception {
        ...
        // Invalid JSON Login
        String badLoginReq =
            "{\"type\":\"LOGIN\",\"requestId\":\"r-1\",\"payload\":\"bad_json\"}";
        ...
        String badRegReq =
            "{\"type\":\"REGISTER\",\"requestId\":\"r-2\",\"payload\":\"bad_json\"}";
        ...
        when(mockAuthService.login(anyString(), anyString())).thenThrow(
            new IllegalArgumentException("Service login error")
        );
        String loginReq =
        ...
        when(
            mockAuthService.register(anyString(), anyString(), anyString())
        ...
        when(mockAdminService.getAllUsers(anyString())).thenThrow(
            new IllegalArgumentException("Admin users error")
        );
        String listUsersReq =
        ...
        when(mockAdminService.getAllAuctions(anyString())).thenThrow(
            new IllegalArgumentException("Admin auctions error")
        );
        String listAuctionsReq =
        ...
        when(mockAdminService.deleteUser(anyString(), anyString())).thenThrow(
            new IllegalArgumentException("Admin delete user error")
        );
        String delUserReq =
        ...
        when(
            mockAdminService.cancelAuction(anyString(), anyString())
        ...
        when(mockAuctionService.createAuction(anyString(), any())).thenThrow(
            new IllegalArgumentException("Create auction error")
        );
        String createReq =
        ...
        when(mockAuctionService.getMyListings(anyString())).thenThrow(
            new IllegalArgumentException("My listings error")
        );
        String listReq =
        ...
        when(mockAutoBidService.saveConfig(any())).thenThrow(
            new IllegalArgumentException("Auto bid config error")
        );
        String configReq =
        ...
        when(mockBidService.placeBid(any(), any(), any(), any())).thenThrow(
        ...
        // List Auctions Service Exception
        when(mockBidService.listAllAuctions()).thenThrow(
            new IllegalArgumentException("List auctions error")
        );
        String listAllReq =
            "{\"type\":\"LIST_AUCTIONS\",\"requestId\":\"r-14\",\"payload\":{}}";
        ...
        when(mockBidService.getAuctionDetail(any())).thenThrow(
            new com.nhom1.auction.common.exception.ValidationException(
        ...
        when(mockBidService.getMyBids(any())).thenThrow(
            new com.nhom1.auction.common.exception.ValidationException(
                "List my bids error"
            )
        );
        ...
        when(
            mockPaymentService.processPayment(anyString(), anyString())
        ...
        when(mockPaymentService.listPendingPayments(anyString())).thenThrow(
            new IllegalArgumentException("Pending payments error")
        );
        String pendingReq =
        ...
        when(mockPaymentService.listPaymentHistory(anyString())).thenThrow(
            new IllegalArgumentException("Payment history error")
        );
        String histReq =
    ...
    public void testMessageRouterEdgeCases() {
        ...
        String unknownTypeJson =
            "{\"type\":\"INVALID_MESSAGE_TYPE_ENUM_VALUE\",\"requestId\":\"r-1\"}";
