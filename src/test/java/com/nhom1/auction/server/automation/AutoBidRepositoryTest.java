...
public class AutoBidRepositoryTest {
  ...
  public void testSave_Success() throws SQLException {
    AutoBidConfig config =
        new AutoBidConfig(
            UUID.randomUUID(), UUID.randomUUID(), new BigDecimal("1000.0"), new BigDecimal("50.0"));
    repo.save(config);
  ...
  public void testFindByAuctionId_ThrowsException() throws SQLException {
    ...
    RuntimeException thrown =
        assertThrows(RuntimeException.class, () -> repo.findByAuctionId(auctionId));
    assertTrue(thrown.getMessage().contains("Failed to read auto bid configs"));
  }

  @Test
  public void testFindByAuctionAndBidder_Success() throws SQLException {
    UUID auctionId = UUID.randomUUID();
    UUID bidderId = UUID.randomUUID();
    ...
    var configOpt = repo.findByAuctionAndBidder(auctionId, bidderId);
    assertTrue(configOpt.isPresent());
    assertEquals(auctionId, configOpt.get().getAuctionId());
    assertEquals(bidderId, configOpt.get().getBidderId());

    // 2. Not found
    when(mockResultSet.next()).thenReturn(false);
    configOpt = repo.findByAuctionAndBidder(auctionId, bidderId);
    assertFalse(configOpt.isPresent());
  }

  @Test
  public void testFindByAuctionAndBidder_ThrowsException() throws SQLException {
    UUID auctionId = UUID.randomUUID();
    UUID bidderId = UUID.randomUUID();
    when(mockConnection.prepareStatement(anyString())).thenThrow(new SQLException("Prep failed"));
    ...
    RuntimeException thrown =
        assertThrows(
            RuntimeException.class, () -> repo.findByAuctionAndBidder(auctionId, bidderId));
    assertTrue(thrown.getMessage().contains("Failed to read auto bid config"));
  }

  @Test
  public void testDeleteByAuctionAndBidder_Success() throws SQLException {
    UUID auctionId = UUID.randomUUID();
    UUID bidderId = UUID.randomUUID();
    when(mockPreparedStatement.executeUpdate()).thenReturn(1);

    int rows = repo.deleteByAuctionAndBidder(auctionId, bidderId);
    assertEquals(1, rows);
  }

  @Test
  public void testDeleteByAuctionAndBidder_ThrowsException() throws SQLException {
    UUID auctionId = UUID.randomUUID();
    UUID bidderId = UUID.randomUUID();
    when(mockConnection.prepareStatement(anyString())).thenThrow(new SQLException("Prep failed"));
    ...
    RuntimeException thrown =
        assertThrows(
            RuntimeException.class, () -> repo.deleteByAuctionAndBidder(auctionId, bidderId));
    assertTrue(thrown.getMessage().contains("Failed to delete auto bid config"));
  }
  ...
  public void testDeleteByAuctionId_Success() throws SQLException {
    ...
    int rows = repo.deleteByAuctionId(auctionId);
    assertEquals(2, rows);
  }
  ...
  public void testDeleteByAuctionId_ThrowsException() throws SQLException {
    ...
    RuntimeException thrown =
        assertThrows(RuntimeException.class, () -> repo.deleteByAuctionId(auctionId));
    assertTrue(thrown.getMessage().contains("Failed to cleanup auto bid configs by auction"));
  }
}
