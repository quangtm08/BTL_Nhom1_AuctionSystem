...
public class ServerConnectionTest {
    ...
    public void testConnectFailureAndFallback() {
        try (var socketMock = mockConstruction(Socket.class, (mock, context) -> {
            doThrow(new IOException("Simulated Cloud Connection Failure")).when(mock).connect(any(), anyInt());
        })) {
    ...
    public void testSendRequest_SuccessAndReceiveResponse() throws Exception {
        ...
        try (var socketMock = mockConstruction(Socket.class, (mock, context) -> {
            ...
            Method handleMethod = ServerConnection.class.getDeclaredMethod("handleRawResponse", String.class);
            handleMethod.setAccessible(true);
            handleMethod.invoke(conn, "{\"requestId\":\"test-req-id\",\"success\":true,\"payload\":\"ok\"}");
    ...
    public void testSendRequest_ExceptionDuringSerialization() throws Exception {
        ...
        doThrow(new RuntimeException("Printer broke")).when(outStream).write(any(byte[].class), anyInt(), anyInt());
    ...
    public void testPushNotificationHandler() throws Exception {
        try (var socketMock = mockConstruction(Socket.class, (mock, context) -> {
            ...
            conn.registerPushHandler(MessageType.PUSH_NEW_AUCTION, json -> {
    ...
    public void testHandleRawResponse_InvalidJsonAndNoPendingRequest() throws Exception {
        try (var socketMock = mockConstruction(Socket.class, (mock, context) -> {
            ...
            assertDoesNotThrow(() -> handleMethod.invoke(conn, "{\"requestId\":\"unknown-req-id\",\"success\":true}"));
    ...
    public void testStartListeningThread_EofAndException() throws Exception {
        ...
        try (var socketMock = mockConstruction(Socket.class, (mock, context) -> {
            ...
            when(badInput.read(any(byte[].class), anyInt(), anyInt())).thenThrow(new IOException("Socket read error"));
