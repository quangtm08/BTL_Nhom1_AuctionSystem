...
public class BaseClientServiceTest {
  ...
  public void testMapServerErrorAllCodes() throws Exception {
    ...
    // 1. Validation Error
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.VALIDATION_ERROR, "msg"))
            instanceof ValidationException);
    // 2. Invalid Format
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.INVALID_FORMAT, "msg"))
            instanceof ValidationException);
    // 3. Authentication Failed
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.AUTHENTICATION_FAILED, "msg"))
            instanceof AuthenticationException);
    // 4. Unauthorized
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.UNAUTHORIZED, "msg"))
            instanceof UnauthorizedActionException);
    // 5. Not Found
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.NOT_FOUND, "msg"))
            instanceof NotFoundException);
    // 6. Invalid Bid
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.INVALID_BID, "msg"))
            instanceof InvalidBidException);
    // 7. Auction Closed
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.AUCTION_CLOSED, "msg"))
            instanceof AuctionClosedException);
    // 8. Invalid Auction State
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.INVALID_AUCTION_STATE, "msg"))
            instanceof InvalidAuctionStateException);
    // 9. Payment Failed
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.PAYMENT_FAILED, "msg"))
            instanceof PaymentException);
    // 10. Conflict
    assertTrue(
        service.publicMapServerError(new ErrorResponse(ErrorCode.CONFLICT, "msg"))
  ...
  public void testSendUnwrapError() {
    ...
    ResponseMessage<String> response =
        new ResponseMessage<>("id", ErrorCode.CONFLICT, "conflict message");
