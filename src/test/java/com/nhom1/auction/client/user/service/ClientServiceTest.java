...
public class ClientServiceTest {
    ...
    public void setUp() throws Exception {
        ...
        Field instanceField = ServerConnection.class.getDeclaredField(
            "instance"
        );
    ...
    public void testAuthClientService() throws Exception {
        ...
        ResponseMessage<AuthResponse> successMsg = new ResponseMessage<>(
            "id",
            loginRes
        );
        CompletableFuture rawFuture = CompletableFuture.completedFuture(
            successMsg
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ...
        assertThrows(CompletionException.class, () ->
            service.login(null, "password").join()
        );
        assertThrows(CompletionException.class, () ->
            service.login("user", "").join()
        );

        // 3. Register success
        ResponseMessage<AuthResponse> regMsg = new ResponseMessage<>(
            "id",
            loginRes
        );
        CompletableFuture rawRegFuture = CompletableFuture.completedFuture(
            regMsg
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawRegFuture);
        AuthResponse respReg = service
            .register("testuser", "email@test.com", "password", "password")
            .get();
        assertEquals("testuser", respReg.getUsername());

        AuthResponse respRegDefaultEmail = service
            .register("testuser2", null, "password", "password")
            .get();
        ...
        assertThrows(CompletionException.class, () ->
            service
                .register("", "email@test.com", "password", "password")
                .join()
        );
        assertThrows(CompletionException.class, () ->
        ...
        assertThrows(CompletionException.class, () ->
            service
                .register("user", "email@test.com", "password", "different")
                .join()
        );
    ...
    public void testAutoBidClientService() throws Exception {
        ...
        ResponseMessage<AutoBidConfigResponse> successMsg =
            new ResponseMessage<>("id", conf);
        CompletableFuture rawFuture = CompletableFuture.completedFuture(
            successMsg
        );
        when(
        ...
        AutoBidConfigResponse resp = service
            .saveConfig(
                "auction-1",
                new BigDecimal("100.00"),
                new BigDecimal("10.00")
    ...
    public void testBiddingClientService() throws Exception {
        ...
        CompletableFuture rawListFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", listRes)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawListFuture);
        assertNotNull(service.listAuctions().get());

        // 2. Get auction detail
        AuctionDetailDto detail = new AuctionDetailDto();
        CompletableFuture rawDetailFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", detail)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawDetailFuture);
        assertNotNull(service.getAuctionDetail("auction-1").get());
        assertThrows(CompletionException.class, () ->
            service.getAuctionDetail(null).join()
        );

        // 3. Place bid validation when not logged in
        assertThrows(CompletionException.class, () ->
            service.placeBid("auc-1", BigDecimal.TEN).join()
        );
        ...
        assertThrows(CompletionException.class, () ->
            service.placeBid(null, BigDecimal.TEN).join()
        );
        assertThrows(CompletionException.class, () ->
            service.placeBid("auc-1", BigDecimal.ZERO).join()
        );

        PlaceBidResponse bidResp = new PlaceBidResponse();
        CompletableFuture rawBidFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", bidResp)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawBidFuture);
        assertNotNull(service.placeBid("auc-1", BigDecimal.TEN).get());

        // 5. Get my bids
        MyBidsResponse myBids = new MyBidsResponse();
        CompletableFuture rawMyBidsFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", myBids)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawMyBidsFuture);
        assertNotNull(service.getMyBids().get());

        // Clear session and test get my bids fails
        AppContext.clearSession();
        assertThrows(CompletionException.class, () ->
            service.getMyBids().join()
        );
    }
    ...
    public void testCreateAuctionClientService() throws Exception {
        ...
        Field uploadServiceField =
            CreateAuctionClientService.class.getDeclaredField(
                "imageUploadService"
            );
        ...
        CompletableFuture rawCreateFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", resp)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawCreateFuture);

        File dummyFile = File.createTempFile("test-upload", ".jpg");
        dummyFile.deleteOnExit();
        when(mockUploadService.upload(dummyFile)).thenReturn(
            CompletableFuture.completedFuture("http://uploaded-url.com/1.jpg")
        );

        CreateAuctionResponse createResp = service
            .createAuction(
                "Title",
                "Desc",
                "10.0",
    ...
    public void testPaymentClientService() throws Exception {
        ...
        assertThrows(CompletionException.class, () ->
            service.listPendingPayments().join()
        );
        ...
        CompletableFuture rawPendingFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", pending)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawPendingFuture);
        ...
        CompletableFuture rawHistoryFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", history)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawHistoryFuture);
        assertNotNull(service.listPaymentHistory().get());

        // 4. Process payment validation
        assertThrows(CompletionException.class, () ->
            service.processPayment(null).join()
        );

        // 5. Process payment success
        ProcessPaymentResponse payResp = new ProcessPaymentResponse();
        CompletableFuture rawPayFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", payResp)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawPayFuture);
    ...
    public void testAdminClientService() throws Exception {
        ...
        assertThrows(CompletionException.class, () ->
            service.listUsers().join()
        );
        assertThrows(CompletionException.class, () ->
            service.listAllAuctions().join()
        );
        assertThrows(CompletionException.class, () ->
            service.deleteUser("user-1").join()
        );
        assertThrows(CompletionException.class, () ->
            service.cancelAuction("auc-1").join()
        );
        ...
        CompletableFuture rawUsersFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", users)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawUsersFuture);
        ...
        CompletableFuture rawAuctionsFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", listAuctions)
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ...
        CompletableFuture rawDelFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", "user-deleted")
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawDelFuture);
        assertEquals("user-deleted", service.deleteUser("user-1").get());
        assertThrows(CompletionException.class, () ->
            service.deleteUser(null).join()
        );

        // 5. Cancel auction success
        CompletableFuture rawCancelFuture = CompletableFuture.completedFuture(
            new ResponseMessage<>("id", "canceled")
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawCancelFuture);
        assertEquals("canceled", service.cancelAuction("auc-1").get());
        assertThrows(CompletionException.class, () ->
            service.cancelAuction(null).join()
        );
    }
    ...
    public void testImageUploadService() throws Exception {
        ...
        Field clientField = ImageUploadService.class.getDeclaredField(
            "httpClient"
        );
        ...
        assertThrows(CompletionException.class, () ->
            service.upload(null).join()
        );
        assertThrows(CompletionException.class, () ->
            service.upload(new File("nonexistent-file.jpg")).join()
        );
        ...
        when(mockResponse.body()).thenReturn(
            "{\"success\":true,\"data\":{\"display_url\":\"https://imgbb.com/test.jpg\"}}"
        );
        ...
        when(mockClient.send(any(), any())).thenThrow(
        ...
        try (
            var propertiesMock = mockConstruction(
            ...
            java.lang.reflect.Method m =
                ImageUploadService.class.getDeclaredMethod("resolveApiKey");
        ...
        try (
            var propertiesMock = mockConstruction(
                ...
                (mockProp, context) -> {
                    doThrow(
                        new java.io.IOException("Simulated property load error")
                    )
                        .when(mockProp)
    ...
    public void testWalletClientService() throws Exception {
        ...
        ResponseMessage<WalletResponse> successMsg = new ResponseMessage<>(
            "id",
            walletRes
        );
        CompletableFuture rawFuture = CompletableFuture.completedFuture(
        ...
        WalletResponse respDep = service
        ...
        AppContext.clearSession();
        assertThrows(CompletionException.class, () ->
            service.deposit(new BigDecimal("500.00")).join()
        );
        AppContext.setCurrentUser(user);
        assertThrows(CompletionException.class, () ->
            service.deposit(null).join()
        );
        assertThrows(CompletionException.class, () ->
            service.deposit(BigDecimal.ZERO).join()
        );
        assertThrows(CompletionException.class, () ->
            service.deposit(new BigDecimal("-10.00")).join()
        );
        ...
        WalletResponse respWith = service
            .withdraw(new BigDecimal("200.00"))
            .get();
        ...
        AppContext.clearSession();
        assertThrows(CompletionException.class, () ->
            service.withdraw(new BigDecimal("200.00")).join()
        );
        AppContext.setCurrentUser(user);
        assertThrows(CompletionException.class, () ->
            service.withdraw(null).join()
        );
        assertThrows(CompletionException.class, () ->
            service.withdraw(BigDecimal.ZERO).join()
        );
        assertThrows(CompletionException.class, () ->
            service.withdraw(new BigDecimal("-5.00")).join()
        );
    }
    ...
    public void testMyListingsClientService() throws Exception {
        ...
        // 1. Not logged in
        assertThrows(CompletionException.class, () ->
            service.listMyListings().join()
        );
        assertThrows(CompletionException.class, () ->
            service.deleteListing("auc-1").join()
        );
        ...
        ResponseMessage<MyListingsResponse> successMsg = new ResponseMessage<>(
            "id",
            myRes
        );
        CompletableFuture rawListFuture = CompletableFuture.completedFuture(
        ...
        ResponseMessage<String> successDel = new ResponseMessage<>(
            "id",
            "deleted"
        );
        CompletableFuture rawDelFuture = CompletableFuture.completedFuture(
        ...
        assertThrows(CompletionException.class, () ->
            service.deleteListing(null).join()
        );
    }
    ...
    public void testEditAuctionClientService() throws Exception {
        ...
        // 4. Update success
        ResponseMessage<String> successUpdate = new ResponseMessage<>(
            "id",
            "updated"
        );
        CompletableFuture rawUpdateFuture = CompletableFuture.completedFuture(
            successUpdate
        );
        when(
            mockConnection.sendRequest(any(RequestMessage.class), any())
        ).thenReturn(rawUpdateFuture);
