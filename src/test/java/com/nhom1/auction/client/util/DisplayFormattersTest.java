...
public class DisplayFormattersTest {
    ...
    public void testPrivateConstructor() throws Exception {
        Constructor<DisplayFormatters> constructor =
            DisplayFormatters.class.getDeclaredConstructor();
    ...
    public void testMoneyOrDash() {
        ...
        assertEquals(
            "$500",
            DisplayFormatters.moneyOrDash(new BigDecimal("500"))
        );
    }
    ...
    public void testTimeLeft() {
        ...
        assertTrue(
            DisplayFormatters.timeLeft(hoursLeft).contains("hours left")
        );

        // Minutes left (less than 1 hour)
        LocalDateTime minutesLeft = LocalDateTime.now().plusMinutes(20);
        assertTrue(
            DisplayFormatters.timeLeft(minutesLeft).contains("min left")
        );
    }
    ...
    public void testAuctionStatusLabel() {
        ...
        assertEquals(
            "Pending",
            DisplayFormatters.auctionStatusLabel(AuctionStatus.PENDING)
        );
        assertEquals(
            "Open",
            DisplayFormatters.auctionStatusLabel(AuctionStatus.OPEN)
        );
        assertEquals(
            "Running",
            DisplayFormatters.auctionStatusLabel(AuctionStatus.RUNNING)
        );
        assertEquals(
            "Ended",
            DisplayFormatters.auctionStatusLabel(AuctionStatus.FINISHED)
        );
        assertEquals(
            "Ended",
            DisplayFormatters.auctionStatusLabel(AuctionStatus.CANCELED)
        );
        assertEquals(
            "Ended",
            DisplayFormatters.auctionStatusLabel(AuctionStatus.PAID)
        );
    }
    ...
    public void testAdminAuctionStatusStyle() {
        assertEquals(
            "status-running",
            DisplayFormatters.adminAuctionStatusStyle(AuctionStatus.RUNNING)
        );
        assertEquals(
            "status-pill-active",
            DisplayFormatters.adminAuctionStatusStyle(AuctionStatus.PAID)
        );
        assertEquals(
            "status-pill-banned",
            DisplayFormatters.adminAuctionStatusStyle(AuctionStatus.CANCELED)
        );
        assertEquals(
            "status-open",
            DisplayFormatters.adminAuctionStatusStyle(AuctionStatus.PENDING)
        );
        assertEquals(
            "status-open",
            DisplayFormatters.adminAuctionStatusStyle(AuctionStatus.OPEN)
        );
        assertEquals(
            "table-text-sub",
            DisplayFormatters.adminAuctionStatusStyle(AuctionStatus.FINISHED)
        );
    }
}
