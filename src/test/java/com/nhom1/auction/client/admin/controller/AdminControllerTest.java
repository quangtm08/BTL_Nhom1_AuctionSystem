...
public class AdminControllerTest {
    ...
    public void testAdminOverviewController() throws Exception {
        ...
        UserSummaryDto user1 = new UserSummaryDto("u-1", "john", "john@gmail.com", UserRole.USER, LocalDateTime.now());
        UserSummaryDto user2 = new UserSummaryDto("u-2", "boss", "boss@gmail.com", UserRole.ADMIN, LocalDateTime.now());
    ...
    public void testAdminOverviewControllerFailure() throws Exception {
        ...
        AdminClientService mockAdminService = mock(AdminClientService.class);
        when(mockAdminService.listUsers()).thenReturn(CompletableFuture.failedFuture(new RuntimeException("API Error")));
        when(mockAdminService.listAllAuctions()).thenReturn(CompletableFuture.completedFuture(new AdminAuctionListResponse()));
    ...
    public void testAdminSidebarControllerEdgeCases() throws Exception {
        ...
        java.lang.reflect.Method mUpdateActive = AdminSidebarController.class.getDeclaredMethod("updateActiveButton");
        ...
        java.lang.reflect.Method mNavigate = AdminSidebarController.class.getDeclaredMethod("navigateWithLoading", AppView.class);
    ...
    public void testAuctionManagementController() throws Exception {
        ...
        // Cancel auction test
        when(mockAdminService.cancelAuction(eq("a-1"))).thenReturn(CompletableFuture.completedFuture("CANCELED"));
        java.lang.reflect.Method cancelMethod = AuctionManagementController.class.getDeclaredMethod("cancelAuction", String.class, Button.class);
    ...
    public void testUserManagementController() throws Exception {
        ...
        AdminUserListResponse ulr = new AdminUserListResponse();
        UserSummaryDto u = new UserSummaryDto("u-1", "sally", "sally@mail.com", UserRole.USER, LocalDateTime.now());
        ...
        // Delete user test
        when(mockAdminService.deleteUser(eq("u-1"))).thenReturn(CompletableFuture.completedFuture("DELETED"));
        java.lang.reflect.Method deleteMethod = UserManagementController.class.getDeclaredMethod("deleteUser", String.class, Button.class);
    ...
    public void testAuctionManagementControllerExceptionsAndPush() throws Exception {
        ...
        org.mockito.ArgumentCaptor<MessageType> msgTypeCaptor = org.mockito.ArgumentCaptor.forClass(MessageType.class);
        ...
        verify(mockConnection, atLeastOnce()).registerPushHandler(msgTypeCaptor.capture(), consumerCaptor.capture());
        ...
        for (int i = 0; i < capturedTypes.size(); i++) {
            if (capturedTypes.get(i) == MessageType.PUSH_NEW_AUCTION ||
                ...
                capturedConsumers.get(i).accept("{}");
                waitForRunLater();
            }
        }
    ...
    public void testUserManagementControllerExceptionsAndPush() throws Exception {
        ...
        // Trigger push handlers and check reload is called
        reset(mockAdminService);
        when(mockAdminService.listUsers()).thenReturn(CompletableFuture.completedFuture(new AdminUserListResponse()));
    ...
    public void testAuctionManagementControllerHelperMethodsAndStatusPills() throws Exception {
        ...
        java.lang.reflect.Method mStatusStyle = AuctionManagementController.class.getDeclaredMethod("statusStyle", AuctionStatus.class);
        ...
        // shortId(String)
        java.lang.reflect.Method mShortId = AuctionManagementController.class.getDeclaredMethod("shortId", String.class);
        ...
        java.lang.reflect.Method mFormatPrice = AuctionManagementController.class.getDeclaredMethod("formatPrice", BigDecimal.class);
        ...
        java.lang.reflect.Method mFormatDateTime = AuctionManagementController.class.getDeclaredMethod("formatDateTime", LocalDateTime.class);
        ...
        assertEquals("22/05/2026 10:30", mFormatDateTime.invoke(controller, time));

        // nvl(String)
        java.lang.reflect.Method mNvl = AuctionManagementController.class.getDeclaredMethod("nvl", String.class);
    ...
    public void testUserManagementControllerHelperMethodsAndAdminRole() throws Exception {
        ...
        java.lang.reflect.Method mFormatDate = UserManagementController.class.getDeclaredMethod("formatDate", LocalDateTime.class);
        mFormatDate.setAccessible(true);
        assertEquals("-", mFormatDate.invoke(controller, (LocalDateTime) null));
        LocalDateTime time = LocalDateTime.of(2026, 5, 22, 10, 30);
        assertEquals("May 22, 2026", mFormatDate.invoke(controller, time));

        // nvl(String)
        java.lang.reflect.Method mNvl = UserManagementController.class.getDeclaredMethod("nvl", String.class);
        mNvl.setAccessible(true);
        assertEquals("-", mNvl.invoke(controller, (String) null));
        assertEquals("-", mNvl.invoke(controller, "   "));
        assertEquals("hello", mNvl.invoke(controller, "hello"));
        ...
        UserSummaryDto adminUser = new UserSummaryDto("u-admin", "admin_boss", "admin@mail.com", UserRole.ADMIN, LocalDateTime.now());
        java.lang.reflect.Method mAddRow = UserManagementController.class.getDeclaredMethod("addRow", int.class, UserSummaryDto.class);
