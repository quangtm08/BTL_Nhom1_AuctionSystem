...
public class AdminControllerTest {
    ...
    public void testAdminOverviewController() throws Exception {
        ...
        when(mockAdminService.listUsers()).thenReturn(
            CompletableFuture.completedFuture(ulr)
        );
        when(mockAdminService.listAllAuctions()).thenReturn(
            CompletableFuture.completedFuture(alr)
        );
        ...
        injectField(
            controller,
            "lblTotalUsersBreakdown",
            lblTotalUsersBreakdown
        );
        injectField(
            controller,
            "lblActiveAuctionsValue",
            lblActiveAuctionsValue
        );
        injectField(
            controller,
            "lblActiveAuctionsBreakdown",
            lblActiveAuctionsBreakdown
        );
    ...
    public void testAdminOverviewControllerFailure() throws Exception {
        ...
        when(mockAdminService.listAllAuctions()).thenReturn(
            CompletableFuture.completedFuture(new AdminAuctionListResponse())
        );
        ...
        assertTrue(
            lblRecentActivityBody
                .getText()
                .contains("Admin dashboard failed to load")
        );
    }
    ...
    public void testAdminSidebarControllerEdgeCases() throws Exception {
        ...
        java.lang.reflect.Method mUpdateActive =
            AdminSidebarController.class.getDeclaredMethod(
                "updateActiveButton"
            );
    ...
    public void testAuctionManagementController() throws Exception {
        AuctionManagementController controller =
            new AuctionManagementController();
        ...
        // Cancel auction test
        when(mockAdminService.cancelAuction(eq("a-1"))).thenReturn(
            CompletableFuture.completedFuture("CANCELED")
        );
    ...
    public void testUserManagementController() throws Exception {
        ...
        // Delete user test
        when(mockAdminService.deleteUser(eq("u-1"))).thenReturn(
            CompletableFuture.completedFuture("DELETED")
        );
        java.lang.reflect.Method deleteMethod =
            UserManagementController.class.getDeclaredMethod(
                "deleteUser",
                String.class,
                Button.class
            );
    ...
    @Test
    public void testAuctionManagementControllerExceptionsAndPush()
        throws Exception {
        AuctionManagementController controller =
            new AuctionManagementController();
        AdminClientService mockAdminService = mock(AdminClientService.class);
        ...
        when(mockAdminService.listAllAuctions()).thenReturn(
            CompletableFuture.failedFuture(
                new RuntimeException("Simulated load error")
            )
        );
        ...
        > consumerCaptor = org.mockito.ArgumentCaptor.forClass(
        ...
        verify(mockConnection, atLeastOnce()).registerPushHandler(
            msgTypeCaptor.capture(),
            consumerCaptor.capture()
        );
        assertTrue(
            lblAuctionSummary.getText().contains("Load auctions failed")
        );
        ...
        List<java.util.function.Consumer<String>> capturedConsumers =
            consumerCaptor.getAllValues();

        for (int i = 0; i < capturedTypes.size(); i++) {
            if (
                capturedTypes.get(i) == MessageType.PUSH_NEW_AUCTION ||
        ...
        // Cancel fails
        reset(mockAdminService);
        when(mockAdminService.cancelAuction(anyString())).thenReturn(
            CompletableFuture.failedFuture(
                new RuntimeException("Simulated cancel error")
            )
        );
    ...
    @Test
    public void testUserManagementControllerExceptionsAndPush()
        throws Exception {
        UserManagementController controller = new UserManagementController();
        AdminClientService mockAdminService = mock(AdminClientService.class);
        injectField(controller, "adminClientService", mockAdminService);
        ...
        reset(mockAdminService);
        when(mockAdminService.listUsers()).thenReturn(
            CompletableFuture.completedFuture(new AdminUserListResponse())
        );
        ...
        for (int i = 0; i < capturedTypes.size(); i++) {
            if (
                capturedTypes.get(i) == MessageType.PUSH_USER_CREATED ||
        ...
        // Delete fails
        reset(mockAdminService);
        when(mockAdminService.deleteUser(anyString())).thenReturn(
            CompletableFuture.failedFuture(
                new RuntimeException("Simulated delete error")
            )
        );
    ...
    @Test
    public void testAuctionManagementControllerHelperMethodsAndStatusPills()
        throws Exception {
        AuctionManagementController controller =
            new AuctionManagementController();
    ...
    @Test
    public void testUserManagementControllerHelperMethodsAndAdminRole()
        throws Exception {
        UserManagementController controller = new UserManagementController();
        ...
        assertEquals(
            "-",
            com.nhom1.auction.client.util.DisplayFormatters.shortDate(
                (LocalDateTime) null
            )
        );
        ...
        assertEquals(
            "May 22, 2026",
            com.nhom1.auction.client.util.DisplayFormatters.shortDate(time)
        );
        ...
        UserSummaryDto adminUser = new UserSummaryDto(
            "u-admin",
            "admin_boss",
            "admin@mail.com",
            UserRole.ADMIN,
        ...
        for (javafx.scene.Node node : userGrid.getChildren()) {
            if (node instanceof javafx.scene.layout.HBox) {
                ...
                if (
                    !actions.getChildren().isEmpty() &&
                    actions.getChildren().get(0) instanceof Button
                ) {
