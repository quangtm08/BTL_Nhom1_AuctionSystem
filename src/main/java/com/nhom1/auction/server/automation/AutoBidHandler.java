...
public class AutoBidHandler {
  ...
  public void register(MessageRouter router) {
    ...
    router.register(
        MessageType.GET_AUTO_BID_CONFIG,
        (requestId, payloadJson) -> {
          try {
            GetAutoBidConfigRequest dto =
                JsonUtil.fromJson(payloadJson, GetAutoBidConfigRequest.class);
            AutoBidConfigDetailResponse response =
                autoBidService.getConfig(dto.getAuctionId(), dto.getBidderId());
            return ResponseFactory.success(requestId, response);
          } catch (Exception e) {
            return ResponseFactory.fromException(requestId, e);
          }
    ...
    router.register(
        MessageType.DELETE_AUTO_BID_CONFIG,
        (requestId, payloadJson) -> {
          try {
            DeleteAutoBidConfigRequest dto =
                JsonUtil.fromJson(payloadJson, DeleteAutoBidConfigRequest.class);
            AutoBidConfigResponse response =
                autoBidService.deleteConfig(dto.getAuctionId(), dto.getBidderId());
            return ResponseFactory.success(requestId, response);
          } catch (Exception e) {
            return ResponseFactory.fromException(requestId, e);
          }
