...
public class BidService {
      ...
      try (Connection connection = dataSource.getConnection()) {
        ...
        try {
          ...
          if (wallet.getBalance().compareTo(amount) < 0) {
            throw new com.nhom1.auction.common.exception.ValidationException(
                "Insufficient wallet balance to place this bid");
          }

          BidTransaction bidTransaction =
              auction.placeBid(bidderId, amount, bidType, LocalDateTime.now());
  ...
  public AuctionDetailDto getAuctionDetail(UUID auctionId) {
    ...
    try {
      bidHistory =
          bidRepository.findByAuctionId(auctionId).stream().map(this::toBidSummaryDto).toList();
    } catch (RuntimeException ex) {
      bidHistory = List.of();
    }
  ...
  public ListAuctionsResponse listAllAuctions() {
    List<AuctionSummaryDto> auctions =
        auctionRepository.findAll().stream()
            .filter(
                a ->
                    a.getStatus() == com.nhom1.auction.common.enums.AuctionStatus.OPEN
                        || a.getStatus() == com.nhom1.auction.common.enums.AuctionStatus.RUNNING)
  ...
  private BidSummaryDto toBidSummaryDto(BidTransaction bidTransaction) {
    String bidderName =
        userRepository
            .findById(bidTransaction.getBidderId())
            .map(u -> u.getUsername())
            .orElse("Unknown");
