...
public class PaymentHandler {
  ...
  public void register(MessageRouter router) {
    ...
    router.register(
        ...
        (requestId, payloadJson) -> {
          try {
            ListPendingPaymentsRequest dto =
                JsonUtil.fromJson(payloadJson, ListPendingPaymentsRequest.class);
    ...
    router.register(
        ...
        (requestId, payloadJson) -> {
          try {
            ListPaymentHistoryRequest dto =
                JsonUtil.fromJson(payloadJson, ListPaymentHistoryRequest.class);
  ...
  private ResponseMessage<ProcessPaymentResponse> handleProcessPayment(
      String requestId, ProcessPaymentRequest dto) {
    try {
      if (dto == null) {
        return ResponseFactory.invalidFormat(requestId, "Missing process payment payload.");
      }
      return ResponseFactory.success(
          requestId, paymentService.processPayment(dto.getAuctionId(), dto.getBidderId()));
    } catch (Exception e) {
      return ResponseFactory.fromException(requestId, e);
    }
  }

  private ResponseMessage<PendingPaymentsResponse> handleListPendingPayments(
      String requestId, ListPendingPaymentsRequest dto) {
    try {
      if (dto == null) {
        return ResponseFactory.invalidFormat(requestId, "Missing pending payments payload.");
      }
      return ResponseFactory.success(
          requestId, paymentService.listPendingPayments(dto.getBidderId()));
    } catch (Exception e) {
      return ResponseFactory.fromException(requestId, e);
    }
  }

  private ResponseMessage<PaymentHistoryResponse> handlePaymentHistory(
      String requestId, ListPaymentHistoryRequest dto) {
    try {
      if (dto == null) {
