...
public class AdminService {
  ...
  public String deleteUser(String targetUserId, String callerId) {
    ...
    try (Connection connection = dataSource.getConnection()) {
      ...
      try {
        auctionRepository.clearHighestBidderByUserId(target.getId(), connection);
        bidRepository.deleteByBidderId(target.getId(), connection);

        List<Auction> sellerAuctions = auctionRepository.findBySellerId(target.getId(), connection);
        for (Auction auction : sellerAuctions) {
          bidRepository.deleteByAuctionId(auction.getId(), connection);
          int deletedAuctions = auctionRepository.deleteById(auction.getId(), connection);
      ...
      throw new RuntimeException("User deletion failed due to database error", e);
    } catch (AppException e) {
      throw e;
    } catch (Exception e) {
  ...
  public String approveAuction(String auctionId, String callerId, String openingDateStr) {
    ...
    try (Connection connection = dataSource.getConnection()) {
      ...
      try {
        boolean updated =
            auctionRepository.updateStartEndAndStatus(
                parsedAuctionId,
                start,
                end,
