...
public class WalletService {
    ...
    public WalletResponse getWallet(UUID userId) {
        ...
        List<WalletTransactionDto> txDtos = txs.stream()
    ...
    public Wallet getOrCreateWallet(UUID userId, Connection conn) {
        return walletRepository.findByUserId(userId, conn).orElseGet(() -> {
            Wallet wallet = new Wallet(userId, new BigDecimal("100000.00"));
    ...
    public void deposit(UUID userId, BigDecimal amount) {
        ...
        WalletTransaction tx = new WalletTransaction(userId, amount, "DEPOSIT", null, "Deposited money via Mock Wallet");
    ...
    public void withdraw(UUID userId, BigDecimal amount) {
        ...
        WalletTransaction tx = new WalletTransaction(userId, amount, "WITHDRAW", null, "Withdrew money from Mock Wallet");
        walletRepository.saveTransaction(tx);

        notificationService.sendWalletUpdate(userId, newBalance);
    }

    public void transfer(UUID fromUserId, UUID toUserId, BigDecimal amount, String referenceId, String description, Connection conn) {
        if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
        ...
        // Save transactions
        WalletTransaction fromTx = new WalletTransaction(fromUserId, amount, "PAYMENT", referenceId, description);
        walletRepository.saveTransaction(fromTx, conn);

        WalletTransaction toTx = new WalletTransaction(toUserId, amount, "RECEIPT", referenceId, description);
