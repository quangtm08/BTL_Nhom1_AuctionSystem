...
public class NotificationService {
  ...
  public void broadcastBidUpdate(
      UUID auctionId, BigDecimal newHighestBid, UUID newHighestBidderId) {
  ...
  public void broadcastAuctionEnded(UUID auctionId, UUID winnerId, BigDecimal finalPrice) {
    AuctionEndedEvent event =
        new AuctionEndedEvent(
            auctionId.toString(), winnerId != null ? winnerId.toString() : null, finalPrice);
    sendPush(MessageType.PUSH_AUCTION_ENDED, event);
  }

  public void broadcastAuctionTimeExtended(UUID auctionId, LocalDateTime newEndTime) {
    AuctionTimeExtendedEvent event =
        new AuctionTimeExtendedEvent(auctionId.toString(), newEndTime, LocalDateTime.now());
    sendPush(MessageType.PUSH_AUCTION_TIME_EXTENDED, event);
  }

  public void broadcastNewAuction(String auctionId, String itemName, BigDecimal startingPrice) {
    NewAuctionEvent event = new NewAuctionEvent(auctionId, itemName, startingPrice);
  ...
  private void sendPush(MessageType type, Object payload) {
    try {
      ...
      System.err.println(
          "[NotificationService] Error sending push notification: " + e.getMessage());
      e.printStackTrace();
    }
  }

  public void sendWalletUpdate(UUID userId, BigDecimal newBalance) {
    WalletUpdateEvent event = new WalletUpdateEvent(userId.toString(), newBalance);
  ...
  private void sendPushToUser(UUID userId, MessageType type, Object payload) {
    try {
      ...
      System.err.println(
          "[NotificationService] Error sending user push notification: " + e.getMessage());
      e.printStackTrace();
    }
  }
}
