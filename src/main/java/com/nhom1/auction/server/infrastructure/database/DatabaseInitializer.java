...
/**
 * Centralized schema bootstrap. Runs idempotent CREATE TABLE IF NOT EXISTS statements once at
 * server startup. Tables are created in FK-dependency order so strict-FK backends (PostgreSQL) do
 * not reject forward references.
 *
 * <p>Order: users -> items -> auctions -> bids -> auto_bid_configs -> payment_transactions ->
 * item_images -> wallets -> wallet_transactions.
 */
public final class DatabaseInitializer {
  ...
  private static final String[] STATEMENTS = {
    ...
    "CREATE INDEX IF NOT EXISTS idx_payment_transactions_auction_id ON"
        + " payment_transactions(auction_id)",
    "CREATE INDEX IF NOT EXISTS idx_payment_transactions_payer_id ON"
        + " payment_transactions(payer_id)",
    "CREATE INDEX IF NOT EXISTS idx_payment_transactions_payee_id ON"
        + " payment_transactions(payee_id)",
    """
