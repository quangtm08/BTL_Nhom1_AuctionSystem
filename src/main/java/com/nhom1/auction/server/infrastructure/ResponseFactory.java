...
public final class ResponseFactory {
  ...
  public static <T> ResponseMessage<T> fromException(String requestId, Throwable throwable) {
    ...
    if (appException != null) {
      return new ResponseMessage<>(requestId, appException.getCode(), appException.getMessage());
    }

    IllegalArgumentException illegalArgumentException = findIllegalArgumentException(throwable);
    if (illegalArgumentException != null) {
      return new ResponseMessage<>(
          requestId, ErrorCode.VALIDATION_ERROR, illegalArgumentException.getMessage());
    }

    logUnexpectedException(requestId, throwable);
    return new ResponseMessage<>(requestId, ErrorCode.SERVER_ERROR, "Unexpected server error");
  }
  ...
  private static void logUnexpectedException(String requestId, Throwable throwable) {
    ...
    LOGGER.log(
        Level.SEVERE, "Unexpected exception while handling requestId=" + requestId, throwable);
  }
}
