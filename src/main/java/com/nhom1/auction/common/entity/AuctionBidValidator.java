...
final class AuctionBidValidator {
  ...
  static void validatePlaceBid(
      Auction auction, UUID bidderId, BigDecimal amount, BidType bidType, LocalDateTime bidTime) {
    if (bidderId == null) {
    ...
    if (auction.getCurrentHighestBid() == null) {
      if (amount.compareTo(auction.getStartingPrice()) < 0) {
        throw new InvalidBidException(
            "The first bid must be at least the starting price of " + auction.getStartingPrice());
      }
      ...
      if (amount.compareTo(requiredMinimum) < 0) {
        throw new InvalidBidException(
            "amount must be at least " + requiredMinimum + " (currentHighestBid + minIncrement)");
      }
    }
  }
}
