...
public class AuctionManagementController {
    ...
    private void renderAuctions(List<AuctionSummaryDto> auctions) {
        ...
        long pending = safeAuctions.stream().filter(a -> a.getStatus() == AuctionStatus.PENDING).count();
        ...
        lblAuctionSummary.setText(safeAuctions.size() + " total auctions - "
                + pending + " pending, " + open + " open, " + running + " running, " + finished + " finished, "
    ...
    private void addRow(int row, AuctionSummaryDto auction) {
        ...
        boolean canCancel = auction.getStatus() == AuctionStatus.PENDING || auction.getStatus() == AuctionStatus.OPEN || auction.getStatus() == AuctionStatus.RUNNING;
        ...
        approveBtn.setDisable(!(auction.getStatus() == AuctionStatus.PENDING));
