...
public class AdminOverviewController {
  ...
  private void renderDashboard(
      AdminUserListResponse usersResponse, AdminAuctionListResponse auctionsResponse) {
    List<UserSummaryDto> users =
        usersResponse.getUsers() != null ? usersResponse.getUsers() : List.of();
    List<AuctionSummaryDto> auctions =
        auctionsResponse.getAuctions() != null ? auctionsResponse.getAuctions() : List.of();
    ...
    long pendingAuctions =
        auctions.stream().filter(auction -> auction.getStatus() == AuctionStatus.PENDING).count();
    long openAuctions =
        auctions.stream().filter(auction -> auction.getStatus() == AuctionStatus.OPEN).count();
    long runningAuctions =
        auctions.stream().filter(auction -> auction.getStatus() == AuctionStatus.RUNNING).count();
    long finishedAuctions =
        auctions.stream().filter(auction -> auction.getStatus() == AuctionStatus.FINISHED).count();
    long paidAuctions =
        auctions.stream().filter(auction -> auction.getStatus() == AuctionStatus.PAID).count();
    long canceledAuctions =
        auctions.stream().filter(auction -> auction.getStatus() == AuctionStatus.CANCELED).count();
    ...
    lblActiveAuctionsBreakdown.setText(
        pendingAuctions
            + " pending | "
            + openAuctions
            + " open | "
            ...
            + paidAuctions
            + " paid");
    lblRecentActivityBody.setText(
        "Loaded "
            + users.size()
            + " users and "
            + auctions.size()
            + " auctions. Current mix: "
            + runningAuctions
            + " running, "
            + canceledAuctions
            + " canceled.");
  ...
  private void updateSessionStatusCircle(
      long runningAuctions, long finishedAuctions, long paidAuctions) {
