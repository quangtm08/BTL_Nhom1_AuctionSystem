...
public class AdminClientService extends BaseClientService {
    ...
    public CompletableFuture<AdminUserListResponse> listUsers() {
        ...
        if (currentUser == null) return validationError("You must sign in as admin before viewing users.");
        return send(new RequestMessage<>(MessageType.ADMIN_LIST_USERS, new AdminListUsersRequest(currentUser.getUserID())),
                AdminUserListResponse.class);
    }

    public CompletableFuture<AdminAuctionListResponse> listAllAuctions() {
        AuthResponse currentUser = requireAdminUser();
        if (currentUser == null) return validationError("You must sign in as admin before viewing auctions.");
        return send(new RequestMessage<>(MessageType.ADMIN_LIST_AUCTIONS, new AdminListAuctionsRequest(currentUser.getUserID())),
                AdminAuctionListResponse.class);
    }

    public CompletableFuture<String> deleteUser(String targetUserId) {
        if (targetUserId == null || targetUserId.isBlank()) return validationError("Target user ID is required.");
        AuthResponse currentUser = requireAdminUser();
        if (currentUser == null) return validationError("You must sign in as admin before deleting users.");
        return send(new RequestMessage<>(MessageType.ADMIN_DELETE_USER,
                new AdminDeleteUserRequest(targetUserId, currentUser.getUserID())), String.class);
    }
    ...
    private AuthResponse requireAdminUser() {
        ...
        if (currentUser == null || currentUser.getUserID() == null || currentUser.getUserID().isBlank()) return null;
