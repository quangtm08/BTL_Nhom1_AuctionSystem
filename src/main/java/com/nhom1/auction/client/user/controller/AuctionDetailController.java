...
public class AuctionDetailController {
    ...
    private final AutoBidClientService autoBidService =
        new AutoBidClientService();
    private final ClientPushService pushService =
    ...
    public void initialize() {
        ...
        if (btnViewBidHistory != null) {
            btnViewBidHistory.setOnAction(e ->
                AppNavigator.navigateTo(AppView.BID_HISTORY_CHART)
            );
        }
        if (btnBack != null) {
        ...
        biddingService
            ...
            .exceptionally(ex -> {
                ...
                Platform.runLater(() -> {
                    setLoadingState(false);
                    System.err.println(
                        "Failed to load auction detail: " + cause.getMessage()
                    );
                });
                return null;
            });
    ...
    private void handleBidUpdatePush(BidUpdateEvent event) {
        ...
        Platform.runLater(() -> {
            ...
            if (bid != null && activeAutoBidCurrentBidLabel != null) {
                activeAutoBidCurrentBidLabel.setText(
                    DisplayFormatters.money(bid)
                );
            }
            clearAutoBidStatus();
        });
        ...
        biddingService
            ...
            .exceptionally(ex -> {
                Throwable cause = BaseClientService.extractFailure(ex);
                System.err.println(
                    "Failed to refresh bid history: " + cause.getMessage()
                );
                return null;
            });
    }
    ...
    private void applyDetail(AuctionDetailDto dto) {
        ...
        if (lblSellerName != null) {
            lblSellerName.setText(
                dto.getSellerName() != null ? dto.getSellerName() : "Unknown"
            );
        }
        ...
        if (lblMinIncrement != null && dto.getMinBidIncrement() != null) {
            lblMinIncrement.setText(
                DisplayFormatters.money(dto.getMinBidIncrement())
            );
        }

        boolean isOwnAuction =
        ...
        boolean canBid =
            dto.getStatus() == AuctionStatus.RUNNING && !isOwnAuction;
        setBidControlsDisabled(!canBid);

        if (isOwnAuction) {
            showBidError("You cannot bid on your own auction.");
        } else if (dto.getStatus() == AuctionStatus.OPEN) {
            showBidError("Auction has not started yet.");
        } else if (dto.getStatus() != AuctionStatus.RUNNING) {
            showBidError("Auction is not accepting bids.");
        } else {
            clearBidError();
        }
    ...
    private void renderBidHistory(List<BidSummaryDto> history) {
        ...
        for (int i = history.size() - 1; i >= 0; i--) {
            ...
            Label time = new Label(
                DisplayFormatters.bidTime(bid.getCreatedAt())
            );
    ...
    private void onPlaceBid() {
        ...
        biddingService
            ...
            .thenAccept(resp ->
                Platform.runLater(() -> handlePlaceBidSuccess(resp))
            )
            .exceptionally(ex -> {
                Throwable cause = BaseClientService.extractFailure(ex);
                String message =
    ...
    private void onConfigureAutoBid() {
        ...
        BigDecimal increment = parseDisplayedMoney(
            lblMinIncrement != null ? lblMinIncrement.getText() : null
        );
        ...
        autoBidService
            ...
            .thenAccept(config ->
                Platform.runLater(() ->
                    showAutoBidDialog(auctionId, increment, config)
                )
            )
            .exceptionally(ex -> {
                Throwable cause = BaseClientService.extractFailure(ex);
    ...
    private void onCancelAutoBid() {
        ...
        autoBidService
            ...
            .thenAccept(resp ->
                Platform.runLater(() -> handleDeleteAutoBid(resp))
            )
            .exceptionally(ex -> {
                Throwable cause = BaseClientService.extractFailure(ex);
                String message =
    ...
    private void showAutoBidDialog(
        ...
        dialog
            ...
            .addAll(
                getClass()
                    .getResource("/css/client/common_ui.css")
                    .toExternalForm(),
                getClass()
                    .getResource("/css/client/auction_detail.css")
                    .toExternalForm()
            );

        dialog.getDialogPane().getButtonTypes().add(ButtonType.CLOSE);
        Button defaultCloseButton = (Button) dialog
            .getDialogPane()
        ...
        Label currentBidLabel = new Label(
            lblCurrentBid != null ? lblCurrentBid.getText() : "$0"
        );
