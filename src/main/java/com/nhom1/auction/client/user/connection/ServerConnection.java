...
public class ServerConnection {
  ...
  private final Map<MessageType, Consumer<String>> pushHandlers = new ConcurrentHashMap<>();
  ...
  private void connect() {
    ...
    try {
      ...
      this.socket.connect(new InetSocketAddress(cloudHost, cloudPort), timeoutMillis);
      ...
      try {
        this.socket = new Socket();
        this.socket.connect(new InetSocketAddress(localHost, localPort), timeoutMillis);
        ...
        new Thread(
            () -> {
              try {
                ...
                if (connected) {
                  System.out.println("DEBUG: Server closed connection (EOF).");
                  this.connected = false;
                }
              } catch (IOException e) {
  ...
  public <T> CompletableFuture<ResponseMessage<T>> sendRequest(
      RequestMessage<?> request, Class<T> responseClass) {
    if (!connected) {
        ...
        .whenComplete(
            (res, ex) -> {
              if (ex instanceof java.util.concurrent.TimeoutException) {
                System.err.println("DEBUG: Request timed out for ID: " + requestId);
    ...
    try {
      ...
      System.out.println(
          ">>> SENDING [" + socket.getInetAddress() + ":" + socket.getPort() + "]: " + json);
      out.println(json);
    } catch (Exception e) {
  ...
  private void handleRawResponse(String json) {
    ...
    try {
      ...
      System.err.println("DEBUG: Error processing server response: " + e.getMessage());
