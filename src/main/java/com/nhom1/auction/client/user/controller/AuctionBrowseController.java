...
public class AuctionBrowseController {
  ...
  public void initialize() {
    if (AppContext.getCurrentUser() != null) {
      ...
      ServerConnection.getInstance()
          .registerPushHandler(
              MessageType.PUSH_NEW_AUCTION, json -> Platform.runLater(this::loadAuctions));
      ServerConnection.getInstance()
          .registerPushHandler(
      ...
      ServerConnection.getInstance()
          .registerPushHandler(
              MessageType.PUSH_AUCTION_ENDED, json -> Platform.runLater(this::loadAuctions));
    }
  }
  ...
  private Parent createAuctionCard(AuctionSummaryDto dto) {
    try {
      ...
      c.bind(
          ...
          formatMoney(resolveDisplayCurrentBid(dto)),
          formatTimeLeft(dto.getEndTime()),
  ...
  private void handleBidUpdatePush(String json) {
    ...
    try {
      ...
      JsonNode node =
          root.has("payload") && !root.get("payload").isNull() ? root.get("payload") : root;
      String auctionId = node.has("auctionId") ? node.get("auctionId").asText() : null;
      if (auctionId == null) return;
      BigDecimal newBid =
          node.has("newHighestBid") ? new BigDecimal(node.get("newHighestBid").asText()) : null;
    ...
    biddingService
        ...
        .thenCombine(
            biddingService
                ...
                .exceptionally(
                    ex ->
                        new com.nhom1.auction.common.dto.bidding.MyBidsResponse(
              ...
              return auctionsResp.getAuctions().stream()
                  ...
                  .filter(
                      a ->
                          currentUserId == null
                              || a.getSellerId() == null
                              || !currentUserId.equals(a.getSellerId()))
                  .toList();
            })
        ...
        .exceptionally(
            ex -> {
              Platform.runLater(
                  () -> showError("Load auctions failed", ex.getCause().getMessage()));
              return null;
            });
  }
