...
public class MyListingsController {
  ...
  private void renderListings(List<AuctionSummaryDto> listings) {
    ...
    for (int i = 0; i < listings.size(); i++)
      listingsGrid.add(createListingCard(listings.get(i)), i % 2, i / 2);
  }

  private Parent createListingCard(AuctionSummaryDto dto) {
    try {
      FXMLLoader loader =
          new FXMLLoader(getClass().getResource("/views/user/components/listing_card.fxml"));
      ...
      throw new RuntimeException("Failed to load listing card component", e);
    }
  }
  ...
  private void handleBidUpdatePush(BidUpdateEvent event) {
    ...
    if (label != null) label.setText(DisplayFormatters.money(event.getNewHighestBid()));
  }
  ...
  private void handleDeleteListing(AuctionSummaryDto dto) {
    ...
    confirm
        .getDialogPane()
        .getStylesheets()
        .add(getClass().getResource("/css/client/my_listings.css").toExternalForm());
    ButtonType yes = new ButtonType("Yes");
    ButtonType no = new ButtonType("No", ButtonData.CANCEL_CLOSE);
    confirm.getButtonTypes().setAll(yes, no);
    ((Button) confirm.getDialogPane().lookupButton(yes)).getStyleClass().add("button-yes");
    ((Button) confirm.getDialogPane().lookupButton(no)).getStyleClass().add("button-no");
    confirm
        .showAndWait()
        .ifPresent(
            selected -> {
              ...
              listingsService
                  .deleteListing(dto.getId())
                  .thenAccept(response -> Platform.runLater(this::loadMyListings))
                  .exceptionally(
                      ex -> {
                        Platform.runLater(
                            () ->
                                renderMessage(
                                    resolveErrorMessage(ex, "Connection error while deleting.")));
                        return null;
                      });
            });
  }
