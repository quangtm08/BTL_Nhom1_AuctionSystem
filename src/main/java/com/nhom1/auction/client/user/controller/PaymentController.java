...
public class PaymentController {
  ...
  private void reload() {
    ...
    paymentClientService
        ...
        .thenAccept(
            snapshot ->
                Platform.runLater(
                    () -> render(snapshot.pendingPayments(), snapshot.paymentHistory())))
        .exceptionally(
            ex -> {
  ...
  private void render(
    ...
    if (pendingPayments.isEmpty()) {
      ...
      pendingPayments.forEach(
          payment -> pendingPaymentsBox.getChildren().add(createPendingRow(payment)));
    }
  ...
  private HBox createHistoryRow(PaymentHistoryEntryDto entry) {
    ...
    badge
        .getStyleClass()
        .add("RECEIVE".equals(entry.getDirection()) ? "badge-receive" : "badge-pay");

    VBox right =
        new VBox(badge, label(DisplayFormatters.money(entry.getAmount()), "history-price"));
  ...
  private record PaymentSnapshot(
      PendingPaymentsResponse pendingPayments, PaymentHistoryResponse paymentHistory) {}
}
